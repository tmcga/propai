...
class FREDClient:
    ...
    async def get_macro_snapshot(self) -> MacroSnapshot:
        ...
        snapshot.inflation_expectations_10yr = await self._latest_value(
        ...
        snapshot.unemployment_trend = await self._compute_trend(
            "national_unemployment", periods=6
        )

        # Housing
        housing_starts_raw = await self._latest_value("housing_starts")
        permits_raw = await self._latest_value("building_permits")
        snapshot.housing_starts_annualized = (
            int(housing_starts_raw * 1000) if housing_starts_raw else None
        )
        snapshot.building_permits_annualized = (
            int(permits_raw * 1000) if permits_raw else None
        )
    ...
    async def _compute_trend(self, series_key: str, periods: int = 6) -> Optional[str]:
